...
void dfs(ll n){
    ...
    visited[n] = 1;
    for(ll p = 1; p <(ll) intervals.size(); p++)
    {
        if(visited[p])
        {
			continue;
		}
        else if(intervals[p].first < intervals[n].first && intervals[n].first < intervals[p].second)
        {
			dfs(p);
		}
        else if(intervals[p].first < intervals[n].second && intervals[n].second < intervals[p].second)
        {
			dfs(p);
		}
    }
