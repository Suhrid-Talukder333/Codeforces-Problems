...
int main()
{
    ...
    int rest[n+1],gym[n+1],contest[n+1];
    rest[0]=0;
    gym[0]=0;
    contest[0]=0;
    for(int i=1; i<=n; i++)
    {
        if(arr[i]==0)
        {
            rest[i]=min(rest[i-1],min(contest[i-1],gym[i-1]))+1;
            contest[i]=contest[i-1]+1;
            gym[i]=gym[i-1]+1;
        }
        else if(arr[i]==1)
        {
            ...
            gym[i]=gym[i-1]+1;
            rest[i]=min(rest[i-1],contest[i-1])+1;
        }
        else if(arr[i]==2)
        {
            contest[i]=contest[i-1]+1;
            rest[i]=min(rest[i-1],gym[i-1])+1;
